<?php 
	...
	class Usuarios extends Controllers{
		...
		public function usuarios()
		{
			...
			$data['page_tag'] = "Usuarios - WSR Integral";
		...
		public function setUsuario(){
			if($_POST){
				...
				if(empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtCorreo']) || empty($_POST['txtTelefono']) || empty($_POST['listRolid']) || empty($_POST['listStatus']) )
				{
		...
		public function getUsuarios()
		{
			if($_SESSION['permisosMod']['r']){
				...
				for ($i=0; $i < count($arrData); $i++){
					...
					if($_SESSION['permisosMod']['r']){
						$btnView = '<button class="btn btn-info btn-sm btnViewUsuario" onClick="fntViewUsuario('.$arrData[$i]['idusuario'].')" title="Ver usuarios"><i class="fas fa-eye"></i></button>';
					}
					if($_SESSION['permisosMod']['u']){
						if(($_SESSION['idUser'] == 1 and $_SESSION['userData']['idrol'] == 1) ||
							($_SESSION['userData']['idrol'] == 2 and $arrData[$i]['idrol'] != 2) ){
					...
					if($_SESSION['permisosMod']['d']){
						if(($_SESSION['idUser'] == 1 and $_SESSION['userData']['idrol'] == 1) ||
							($_SESSION['userData']['idrol'] == 2 and $arrData[$i]['idrol'] != 2) and
		...
		public function putDFiscal(){
			if($_POST){
				if(empty($_POST['txtRfc']) || empty($_POST['txtNombreSocial']) || empty($_POST['txtCalle']) || empty($_POST['txtNInterior']) || empty($_POST['txtNExterno']) || empty($_POST['txtColonia']) || empty($_POST['txtCP']) || empty($_POST['txtMunicipio']) || empty($_POST['txtEstado']) || empty($_POST['txtPais']) )
				{
					$arrResponse = array("status" => false, "msg" => 'Datos incorrectos.');
				}else{
					$idUsuario = $_SESSION['idUser'];
					...
					$intInterior = intval(strClean($_POST['txtNInterior']));
					$intExterior = intval(strClean($_POST['txtNExterno']));
